...
$('#font_family li, #font_size li, #font_style li, #font_weight li, #list_type li, #list_position li').click ( function() {
        $('#content').val( $('#content').val() + '\t' + $(this).text() + ';\n' );
});

$('#color_attr, #border_attr, #html_tags, #font_attr, #margins_pads_attr, #list_attr').change ( function() {
	if($(this).val()==0) {
		...
		if( $(this).text() == $('#color_attr').text() ) {
			...
			switch( $('option:selected', this).val() ) {
				...
				var attrValue = $('option:selected', this).text() + ": ";
                        	$('#content').val( $('#content').val() + '\t' + attrValue );
                                $('#font_family').show();
				$('#font_size, #font_style, #font_weight, #list_type, #list_position').hide();
				$('#colors, #border_style, #sections, #groupings, #embeddings, #tables, #forms, #hyperlinks').hide();
                                break;
                        case '2':
				var attrValue = $('option:selected', this).text() + ": ";
	                        $('#content').val( $('#content').val() + '\t' + attrValue );
                                $('#font_size').show();
				$('#font_family, #font_style, #font_weight, #list_type, #list_position').hide();
				$('#colors, #border_style, #sections, #groupings, #embeddings, #tables, #forms, #hyperlinks').hide();
                                break;
                        case '3':
				var attrValue = $('option:selected', this).text() + ": ";
	                        $('#content').val( $('#content').val() + '\t' + attrValue );
                                $('#font_weight').show();
				$('#font_family, #font_size, #font_style, #list_type, #list_position').hide();
				$('#colors, #border_style, #sections, #groupings, #embeddings, #tables, #forms, #hyperlinks').hide();
                                break;
                        case '4':
				var attrValue = $('option:selected', this).text() + ": ";
	                        $('#content').val( $('#content').val() + '\t' + attrValue );
                                $('#font_style').show();
				$('#font_family, #font_size, #font_weight, #list_type, #list_position').hide();
				$('#colors, #border_style, #sections, #groupings, #embeddings, #tables, #forms, #hyperlinks').hide();
                                break;
                        default:
				$('#font_family, #font_size, #font_style, #font_weight, #list_type, #list_position').hide();
			...
			var attrValue = $('option:selected', this).text() + ": ";
                        ...
                        case '1':
                                ...
                                $('#content').val( $('#content').val() + '\t' + attrValue );
                                $('#list_type').show();
                                $('#font_family, #font_size, #font_style, #font_weight, #list_position').hide();
                                $('#colors, #border_style, #sections, #groupings, #embeddings, #tables, #forms, #hyperlinks').hide();
                                break;
                        case '2':
                                var attrValue = $('option:selected', this).text() + ": ";
                                $('#content').val( $('#content').val() + '\t' + attrValue );
                                $('#list_position').show();
                                $('#font_family, #font_size, #font_style, #font_weight, #list_type').hide();
                                $('#colors, #border_style, #sections, #groupings, #embeddings, #tables, #forms, #hyperlinks').hide();
                                break;
                        default:
                                $('#font_family, #font_size, #font_style, #font_weight, #list_type, #list_position').hide();
